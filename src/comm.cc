...
bool
isClosing(const int fd)
{
    return fd >= 0 && fd_table && fd_table[fd].closing();
}
...
comm_connect_addr(int sock, const Ip::Address &address)
{
    ...
    if (!F->flags.called_connect) {
		...
		#if 0
        {
            ...
            if (!getsockopt(sock, IPPROTO_TCP, TCP_FUNCTION_BLK, buf, &len)){
                debugs(98,1, "bef tcp_function_blk: " << buf);
            }
        }
		#endif

        if (Config.onoff.use_bbr){   // BBR
            static struct tcp_function_set tfs = {.function_set_name = {'b','b','r','\0'}, .pcbcnt = 0};
            setsockopt(sock, IPPROTO_TCP, TCP_FUNCTION_BLK, &tfs, sizeof(tfs));
        }
		...
		#if 0
        {
            ...
            socklen_t len = sizeof(buf);
            if (!getsockopt(sock, IPPROTO_TCP, TCP_FUNCTION_BLK, buf, &len)){
                debugs(98,1, "aft tcp_function_blk: " << buf);
            }
        }
		#endif
        #endif
